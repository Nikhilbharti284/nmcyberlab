...
describe('/#/contact', () => {
    ...
    describe('challenge "forgedFeedback"', () => {
        ...
        it('should be possible to provide feedback as another user', () => {
            ...
            cy.get('.customer-table > .mat-mdc-table > :nth-child(8) > .cdk-column-user').then(($val) => {
                if ($val.text() !== ' 2') {
                    cy.get('.customer-table > .mat-mdc-table > :nth-child(9) > .cdk-column-user').should('contain.text', '2');
                }
                else {
    ...
    describe('challenge "typosquattingAngular"', () => {
        it('should be possible to post typosquatting NPM package as feedback', () => {
            cy.get('#rating').type('{rightarrow}{rightarrow}{rightarrow}');
            cy.get('#comment').type('You are a typosquatting victim of this NPM package: ngy-cookie');
            cy.get('#submitButton').should('not.be.disabled').click();
    ...
    describe('challenge "zeroStars"', () => {
        it('should be possible to post feedback with zero stars by double-clicking rating widget', () => {
            ...
            cy.window().then(async () => {
                const response = await fetch(`${Cypress.config('baseUrl')}/rest/captcha/`, {
                    method: 'GET',
                    cache: 'no-cache',
                    headers: {
                ...
                async function sendPostRequest(captcha) {
                    const response = await fetch(`${Cypress.config('baseUrl')}/api/Feedbacks`, {
                        method: 'POST',
                        cache: 'no-cache',
                        headers: {
    ...
    describe('challenge "dlpPastebinDataLeak"', () => {
        it('should be possible to post dangerous ingredients of unsafe product as feedback', () => {
            cy.get('#rating').type('{rightarrow}{rightarrow}{rightarrow}');
            cy.task('GetPastebinLeakProduct').then((pastebinLeakProduct) => {
                cy.get('#comment').type(pastebinLeakProduct.keywordsForPastebinDataLeakChallenge ? pastebinLeakProduct.keywordsForPastebinDataLeakChallenge.toString() : '?');
            });
            cy.get('#submitButton').should('not.be.disabled').click();
